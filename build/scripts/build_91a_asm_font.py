...
def scale_and_annotate(image_binarized, base_font_src_dir, lines_of_text, line_starts, character_height,scale_method):
    ...
    font_path = "src/assets/img/ui/fonts/Arial Black.ttf"
    font_size = 32
...
def scale_font_images(font_def_file, base_font_src_dir, font_rgba2_dir, scale_factor, scale_method, fr_bg, fr_fg, to_bg, to_fg):   
    ...
    for index, row in df.iterrows():
        ...
        # # Convert the image to Agon64 palette TODO: should be redundant since we go to rgba2
        # font_img = au.convert_to_palette(font_img, 64, 'HSV', to_bg)

        # Save the scaled image
        font_img.save(os.path.join(scaled_png_dir, row['img_filename']))

        # Convert the image to Agon rgba2 format
        au.img_to_rgba2(os.path.join(scaled_png_dir, row['img_filename']), os.path.join(font_rgba2_dir, row['img_filename'].replace('.png', '.rgba2')))
...
def main(font_name, abbr_name, lines_of_text, line_starts, character_height, buffer_id_counter, space_width, scale_method):
    base_font_src_dir = f'src/assets/img/ui/fonts/{font_name}'
    source_img_path = f"{base_font_src_dir}/{font_name}.png"
    threshold = 128

    fr_bg = (255,255,255,255) # White
    fr_fg = (0,0,0,255) # Black

    to_bg = (0,0,0,0) # Transparent
    to_fg = (255,255,255,255) # White
