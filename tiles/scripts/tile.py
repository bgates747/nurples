...
def chop_and_deduplicate_tiles(source_dir, target_dir, base_name, file_name, tile_width, tile_height, h_pitch, v_pitch, tiles_x, tiles_y, start_x, start_y, set_num):
    # The source file and metadata stay in target_dir, images go in a subdir named after set_num
    ...
    for row in range(tiles_y):
        ...
        for col in range(tiles_x):
            ...
            if tile_hash not in unique_tiles:
                output_path = os.path.join(level_dir, f"{base_name}_{set_num_hex}_{tile_count:02X}.png")
    ...
    # Metadata stays alongside the source PNG file, not in the image directory.
    ...
    # The TargetDir remains the top-level directory with the source PNG.
...
def generate_combined_xml(combined_xml_filepath, base_name, target_dir, bufferId):
    # Recursively find *_files.xml in all subdirectories
    xml_files = []
...
def generate_asm_img_load(combined_xml_filepath, asm_images_filepath, asm_img_dir):
    ...
    asm_file_list = []
    total_images = 0
    # base_name will be fetched per tile_set
    ...
    if base_name_for_header is None:
        base_name_for_header = base_name  # fallback
    with open(asm_images_filepath, "w") as asm_images_file:
        asm_images_file.write(f"; Generated by tiles_{base_name_for_header.lower()}.py\n\n")
        asm_images_file.write(f"tiles_{base_name_for_header.lower()}_num_images: equ {total_images}\n\n")
        asm_images_file.write("; bufferIds:\n")
        asm_images_file.write("\n".join(asm_bufferIds) + "\n\n")
        asm_images_file.write(f"tiles_{base_name_for_header.lower()}_image_list: ; type; width; height; size; filename; bufferId:\n")
        asm_images_file.write("\n".join(asm_image_list) + "\n\n")
        asm_images_file.write(f"tiles_{base_name_for_header.lower()}_files_list: ; filename:\n")
        asm_images_file.write("\n".join(asm_file_list) + "\n")
...
def generate_asm_levels(combined_xml_filepath, asm_levels_filepath):
    ...
    levels_list = []
    levels_data = []
    # We'll retrieve base_name from the first tile set encountered
    base_name_for_header = None
    ...
    with open(asm_levels_filepath, "w") as asm_levels_file:
        asm_levels_file.write(f"; Generated by tiles_{base_name_for_header.lower()}_levels.py\n\n")
        asm_levels_file.write(f"tiles_{base_name_for_header.lower()}_num_levels: db {num_levels}\n\n")
        asm_levels_file.write(f"tiles_{base_name_for_header.lower()}_levels:\n")
...
def main(bufferId, tile_width, tile_height, h_pitch, v_pitch, base_name, tiles_x, tiles_y, ranges):
    ...
    target_dir = f"tiles/{base_name}"  # The source file and metadata remain here.

    # DO NOT remove target_dir now. Just ensure it exists. It should already exist with the source png.
    # If needed, handle old directories for images on a per-set basis in chop_and_deduplicate_tiles().
...
# Changes made:
# - Do not delete target_dir at the start of main().
# - Images go into target_dir/<set_num> directories; metadata files (map and files.xml) stay in target_dir.
# - Removed references that previously stored metadata inside the subdirectories. Instead, only images are stored in subdirs.
# - Ensured that each call to chop_and_deduplicate_tiles removes and recreates the set_num directory for images, leaving target_dir intact.
